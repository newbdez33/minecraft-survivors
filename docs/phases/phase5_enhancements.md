...
**难度目标**: 新玩家约5分钟内结束 (300秒)

**敌人属性 (Phase 5 调整):**
| 敌人 | HP | 伤害 | 速度 | 特殊能力 | XP |
|------|-----|------|------|----------|-----|
| Zombie | 35 | **15** | **50** | 无 | 5 |
| Skeleton | 30 | **12** | **60** | 远程射箭 | 8 |
| Spider | 25 | **12** | **100** | 跳跃攻击 | 6 |
| **Creeper** | **50** | **50** | **55** | **爆炸(范围120)** | **15** |
| **Enderman** | **60** | **25** | **100** | **受伤传送+愤怒** | **20** |
| Witch | 25 | **15** | **45** | 投掷毒药水 | 12 |
...
- 速度: **100** (非常快)
- 传送冷却: **1.5s** (更频繁)
- **愤怒机制** - 被攻击后5秒内攻击力×1.5 (37.5伤害!)
- 传送后立即攻击 (无攻击冷却)
- 低于30% HP时每次传送恢复5 HP

**敌人生成规则 (更激进):**
| 波数 | 时间 | 新增敌人 | 生成间隔 | 最大数量 |
|------|------|----------|----------|----------|
| 1 | 0s | Zombie | **1.5s** | **40** |
| 2 | 45s | +Skeleton | **1.3s** | **50** |
| 3 | 90s | +Spider | **1.1s** | **60** |
| 4 | 135s | +Creeper | **0.9s** | **70** |
| 5 | 180s | +Enderman | **0.8s** | **80** |
| 6 | 225s | +Witch | **0.7s** | **90** |
| 7+ | 270s | 精英敌人 | **0.6s** | **100** |

**波数时间线 (新玩家预期死亡点):**
```
0:00 ─── 波1开始 (只有僵尸)
0:45 ─── 波2开始 (加入骷髅，开始有压力)
1:30 ─── 波3开始 (加入蜘蛛，速度快难躲)
2:15 ─── 波4开始 (加入苦力怕，爆炸致命)
3:00 ─── 波5开始 (加入末影人，传送骚扰)
3:45 ─── 波6开始 (加入女巫，中毒DOT)
4:30 ─── 波7开始 (精英敌人，难度飙升)
      ↑
      新玩家通常在此之前死亡 (3-5分钟)
5:00 ─── 目标死亡时间
...
### 5.1 Poison Status Effect (中毒状态效果) ✅ IMPLEMENTED

**描述**: 女巫投掷的药水瓶落地后，形成方形像素风毒云区域，玩家进入区域中毒，多次中毒可叠加伤害。

**毒云规格** (PoisonCloud):
| 属性 | 值 | 描述 |
|------|-----|------|
| cloud_size | 80.0 | 方形毒云大小 (80x80像素) |
| cloud_duration | 5.0 | 毒云持续时间 (秒) |
| cloud_damage | 5 | 进入毒云的初始伤害 |
| poison_duration | 3.0 | 中毒持续时间 (秒) |
| poison_damage_per_tick | 2 | 每次中毒伤害 |
| poison_tick_interval | 0.5 | 中毒伤害间隔 (秒) |

**中毒叠加机制** ✅:
- 多次进入毒云，中毒效果叠加
- 每次叠加 +2 伤害/tick
- 叠加次数无上限 (stack_count 记录)
- 刷新持续时间

**视觉效果** ✅:
| 效果 | 描述 |
|------|------|
| 毒云外观 | 方形像素风格 (Minecraft风)，随机绿色像素格子 |
| 毒云粒子 | 6个浮动绿色方块在云内 |
| 毒云消失 | 渐变淡出 (最后1秒) |
| 中毒指示 | Steve脉冲绿色闪烁 (亮绿↔暗绿交替) |
| 粒子效果 | 4个绿色小方块从玩家身上向上飘浮 |
