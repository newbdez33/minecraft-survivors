...
# Mouse control
const MOUSE_ARRIVE_THRESHOLD: float = 15.0  # Stop when this close to mouse
...
func get_input_direction() -> Vector2:
	...
	# Mouse movement - follow mouse while left button is held
	if Input.is_mouse_button_pressed(MOUSE_BUTTON_LEFT):
		var mouse_pos = get_global_mouse_position()
		var to_mouse = mouse_pos - global_position
		if to_mouse.length() > MOUSE_ARRIVE_THRESHOLD:
