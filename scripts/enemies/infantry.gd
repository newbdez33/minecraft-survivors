...
class_name Infantry
## Infantry (步兵) - Basic melee enemy that chases the player
...
@export var meat_drop_chance: float = 0.0  # Infantry don't drop steamed buns
...
func _setup_animator() -> void:
	...
	if sprite:
		...
		# Infantry-specific animation settings: slow, stiff movement
...
func _on_died() -> void:
	...
	# Elite War Cry: 50% chance to request spawner to spawn 2 normal infantry
