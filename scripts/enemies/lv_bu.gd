...
class_name LvBu
## Lv Bu (吕布) Boss (Wave 30) - Final boss with sky halberd and dive attacks
...
func _setup_animator() -> void:
	...
	if sprite:
		...
		# Lv Bu-specific animation: majestic flying motion
		...
		# Lv Bu is always flying - start animation immediately
...
func _do_fireball_attack() -> void:
	...
	if target and is_instance_valid(target):
		# Spawn sky halberd projectile
		var direction = (target.global_position - global_position).normalized()
		_spawn_fireball(direction)  # Spawn sky halberd

	_state = State.FLYING

func _spawn_fireball(direction: Vector2) -> void:
	# Spawn sky halberd projectile
	var fireball_scene = load("res://scenes/projectiles/sky_halberd.tscn")
	if fireball_scene:
...
func _spawn_death_effect() -> void:
	...
	if death_scene and get_tree() and get_tree().current_scene:
		# Dramatic death for Lv Bu, the final boss
		for i in range(8):
...
func _spawn_drops() -> void:
	_spawn_xp_orbs()
	_spawn_emeralds()
	# Guaranteed special item drop for defeating final boss
	_spawn_special_drop()
...
func _spawn_special_drop() -> void:
	# Guaranteed special drop for defeating Lv Bu
