...
class_name Cavalry
## Cavalry (骑兵) - Fast enemy that can charge toward the player
...
@export var meat_drop_chance: float = 0.0  # Cavalry don't drop steamed buns
...
func _setup_animator() -> void:
	...
	if sprite:
		...
		# Cavalry-specific animation settings: fast, twitchy
...
func jump() -> void:
	...
	if audio:
		audio.play_sfx_at("cavalry_charge", global_position)
...
func _on_hitbox_body_entered(body: Node2D) -> void:
	if body.is_in_group("player") and body.has_method("take_damage"):
		...
		# Elite Poisoned Lance: apply poison on hit
		if is_elite:
