...
class_name XiahouDun
## Xiahou Dun (夏侯惇) Boss - Summons Shadow Guards and casts fang attacks
...
var _guard_count: int = 0
const MAX_GUARDS: int = 3
var _active_guards: Array = []  # Track spawned shadow guards for cleanup
...
func _setup_animator() -> void:
	...
	if sprite:
		...
		# Xiahou Dun-specific: flowing robes, graceful motion
...
func _process_idle(_delta: float) -> void:
	...
	if _guard_count < MAX_GUARDS and _summon_timer >= summon_cooldown:
		_start_summon()
		return
...
func cast_fang_attack() -> void:
	...
	var direction = (target.global_position - global_position).normalized()
	var start_pos = global_position + direction * 50  # Start 50px from Xiahou Dun
...
func summon_shadow_guard() -> void:
	if _guard_count >= MAX_GUARDS:
		return

	var guard_scene = load("res://scenes/enemies/shadow_guard.tscn")
	if not guard_scene:
		return

	var guard = guard_scene.instantiate()

	# Spawn near Xiahou Dun with random offset
	var offset = Vector2(randf_range(-50, 50), randf_range(-50, 50))
	guard.global_position = global_position + offset

	# Connect to shadow guard death to track count
	if guard.has_signal("died"):
		guard.died.connect(_on_guard_died.bind(guard))

	# Track active shadow guards for cleanup
...
func _on_guard_died(_xp: int, guard: Node = null) -> void:
	_guard_count = max(0, _guard_count - 1)
	# Remove from tracking array
	if guard:
		_active_guards.erase(guard)


func _exit_tree() -> void:
	# Disconnect signals from any remaining shadow guards to prevent memory leaks
	for guard in _active_guards:
		if is_instance_valid(guard) and guard.has_signal("died"):
			var callable = _on_guard_died.bind(guard)
			if guard.died.is_connected(callable):
				guard.died.disconnect(callable)
