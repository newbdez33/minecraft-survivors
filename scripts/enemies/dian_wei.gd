...
class_name DianWei
## Dian Wei (典韦) Boss (Wave 20) - Sonic boom, ground slam, darkness aura, and anger tracking
...
func _setup_animator() -> void:
	...
	if sprite:
		...
		# Dian Wei-specific: heavy, breathing motion
...
func _track_by_sound(_delta: float) -> void:
	# Dian Wei detects player by sound/movement
...
func _do_ground_slam() -> void:
	...
	# Play boss stomp animation (reuse Zhang Liao pattern)
	if _animator:
		_animator.stop_walk_animation()
...
func _spawn_slam_effect() -> void:
	# Visual slam effect - 5 hit effects in a ring (dark teal), larger than Zhang Liao's
	var hit_scene = load("res://scenes/effects/hit_effect.tscn")
