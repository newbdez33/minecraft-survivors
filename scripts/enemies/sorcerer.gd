...
class_name Sorcerer
## Sorcerer (术士) - Ranged enemy that throws potions at the player
...
@export var meat_drop_chance: float = 0.0  # Sorcerers don't drop steamed buns
...
func _setup_animator() -> void:
	...
	if sprite:
		...
		# Sorcerer-specific animation settings: graceful, flowing
...
func throw_potion() -> void:
	...
	if audio:
		audio.play_sfx_at("sorcerer_poison_throw", global_position)
	# Load potion scene
...
func _spawn_death_effect() -> void:
	...
	if death_scene and get_tree() and get_tree().current_scene:
		...
		poof.modulate = Color(0.3, 0.6, 0.3)  # Green tint for Sorcerer
