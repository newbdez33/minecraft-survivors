...
class_name FireSoldier
## Fire Soldier (火兵) - Explosive enemy that detonates when near player
...
@export var meat_drop_chance: float = 0.0  # Fire Soldiers don't drop steamed buns
...
func _setup_animator() -> void:
	...
	if sprite:
		...
		# Fire Soldier-specific animation settings: slow swaying
...
func make_elite() -> void:
	is_elite = true
	# Elite Charged fire explosion: wider radius
...
func explode() -> void:
	...
	# Elite Charged Fire Explosion: chain damage to nearby enemies
	if is_elite:
	...
	if audio:
		audio.play_sfx_at("fire_explosion", global_position)
